...
def load_data(data_file_path, d_labels_path):
    features = np.load(data_file_path)
    labels =  np.load(d_labels_path)
...
class SONG(Dataset):
    def __init__(self, data_file_path_mel,data_file_path_co, d_labels_path_a, d_labels_path_v, train=True):
        ...
        # 归一化数据
        #self.feature_mel = (self.feature_mel - np.mean(self.feature_mel)) / np.std(self.feature_mel)
        #self.feature_co = (self.feature_co - np.mean(self.feature_co)) / np.std(self.feature_co)
